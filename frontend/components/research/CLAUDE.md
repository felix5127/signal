...
- **Simple Navbar**: Logo (Radar 图标 + Signal) + 设置按钮 + PRO 徽章
...
- `outputs`: 输出列表
- `chatMessages`: 对话消息历史 (ChatMessage[]，从 API 加载)
...
- POST /api/research/projects/{id}/chat (SSE)
- GET /api/research/projects/{id}/chat/messages
- POST /api/research/projects/{id}/messages/{msgId}/star
- POST /api/research/projects/{id}/generate/report (SSE)
- POST /api/research/projects/{id}/generate/mindmap (SSE)
- GET /api/research/projects/{id}/outputs
- DELETE /api/research/outputs/{id}
...
### 2026-02-09 - SSE 流式集成
- workspace.tsx: 新增 fetchMessages 加载对话历史，移除 sessionId 状态
- workspace.tsx: ChatPanel 新增 onRefreshMessages 回调
- 全部面板使用 useSSE Hook 进行流式通信
- API 端点更新: 对话/报告/思维导图均改为 SSE 流式
